 ...
 * 
 */
public class FinalStateFinalStatePropertiesEditionComponent extends
	 ...
	 * 
	 */
	public FinalStateFinalStatePropertiesEditionComponent(
			final PropertiesEditingContext editingContext,
			final EObject finalState, final String editing_mode) {
		super(editingContext, finalState, editing_mode);
		this.parts = new String[] { FINALSTATE_PART };
		this.repositoryKey = StatemachineViewsRepository.class;
		this.partKey = StatemachineViewsRepository.FinalState.class;
	}
	...
	public void initPart(final Object key, final int kind, final EObject elt,
		...
		if ((this.editingPart != null) && (key == this.partKey)) {
			...
			final FinalStatePropertiesEditionPart finalStatePart = (FinalStatePropertiesEditionPart) this.editingPart;
			// init values
			if (this.isAccessible(StatemachineViewsRepository.FinalState.Properties.description)) {
				finalStatePart.setDescription(EEFConverterUtil.convertToString(
						EcorePackage.Literals.ESTRING,
						finalState.getDescription()));
			}

			if (this.isAccessible(StatemachineViewsRepository.FinalState.Properties.keywords)) {
				finalStatePart.setKeywords(finalState.getKeywords());
			}

			if (this.isAccessible(StatemachineViewsRepository.FinalState.Properties.incomingTransitions)) {
			...
			if (this.isAccessible(StatemachineViewsRepository.FinalState.Properties.outcomingTransitions)) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		final FinalState finalState = (FinalState) this.semanticObject;
		if (StatemachineViewsRepository.FinalState.Properties.description == event
				.getAffectedEditor()) {
			finalState.setDescription((java.lang.String) EEFConverterUtil
					.createFromString(EcorePackage.Literals.ESTRING,
							(String) event.getNewValue()));
		}
		if (StatemachineViewsRepository.FinalState.Properties.keywords == event
				.getAffectedEditor()) {
		...
		if (StatemachineViewsRepository.FinalState.Properties.incomingTransitions == event
				.getAffectedEditor()) {
		...
		if (StatemachineViewsRepository.FinalState.Properties.outcomingTransitions == event
				.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof Transition) {
	...
	public void updatePart(final Notification msg) {
		...
		if (this.editingPart.isVisible()) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description()
					...
					&& (finalStatePart != null)
					&& this.isAccessible(StatemachineViewsRepository.FinalState.Properties.description)) {
				if (msg.getNewValue() != null) {
					finalStatePart.setDescription(EcoreUtil.convertToString(
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Keywords()
					...
					&& msg.getNotifier().equals(this.semanticObject)
					&& (finalStatePart != null)
					&& this.isAccessible(StatemachineViewsRepository.FinalState.Properties.keywords)) {
				if (msg.getNewValue() instanceof EList<?>) {
					finalStatePart.setKeywords((EList<?>) msg.getNewValue());
				} else if (msg.getNewValue() == null) {
					finalStatePart.setKeywords(new BasicEList<Object>());
				} else {
					final BasicEList<Object> newValueAsList = new BasicEList<Object>();
					newValueAsList.add(msg.getNewValue());
					finalStatePart.setKeywords(newValueAsList);
				}
			}
			...
			if (StateMachinePackage.eINSTANCE
					.getAbstractState_IncomingTransitions().equals(
							msg.getFeature())
					&& this.isAccessible(StatemachineViewsRepository.FinalState.Properties.incomingTransitions)) {
				finalStatePart.updateIncomingTransitions();
			}
			if (StateMachinePackage.eINSTANCE
					.getAbstractState_OutcomingTransitions().equals(
							msg.getFeature())
					&& this.isAccessible(StatemachineViewsRepository.FinalState.Properties.outcomingTransitions)) {
