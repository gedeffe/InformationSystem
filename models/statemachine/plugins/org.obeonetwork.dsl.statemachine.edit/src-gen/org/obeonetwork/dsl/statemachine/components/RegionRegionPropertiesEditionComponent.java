 ...
 * 
 */
public class RegionRegionPropertiesEditionComponent extends
	 ...
	 * 
	 */
	public RegionRegionPropertiesEditionComponent(
			final PropertiesEditingContext editingContext,
			final EObject region, final String editing_mode) {
		super(editingContext, region, editing_mode);
		this.parts = new String[] { REGION_PART };
		this.repositoryKey = StatemachineViewsRepository.class;
		this.partKey = StatemachineViewsRepository.Region.class;
	}
	...
	public void initPart(final Object key, final int kind, final EObject elt,
		...
		if ((this.editingPart != null) && (key == this.partKey)) {
			...
			final RegionPropertiesEditionPart regionPart = (RegionPropertiesEditionPart) this.editingPart;
			// init values
			if (this.isAccessible(StatemachineViewsRepository.Region.Properties.description)) {
				regionPart
						.setDescription(EEFConverterUtil.convertToString(
								EcorePackage.Literals.ESTRING,
								region.getDescription()));
			}

			if (this.isAccessible(StatemachineViewsRepository.Region.Properties.keywords)) {
				regionPart.setKeywords(region.getKeywords());
			}

			if (this.isAccessible(StatemachineViewsRepository.Region.Properties.name)) {
				regionPart.setName(EEFConverterUtil.convertToString(
						EcorePackage.Literals.ESTRING, region.getName()));
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		final Region region = (Region) this.semanticObject;
		if (StatemachineViewsRepository.Region.Properties.description == event
				.getAffectedEditor()) {
			region.setDescription((java.lang.String) EEFConverterUtil
					.createFromString(EcorePackage.Literals.ESTRING,
							(String) event.getNewValue()));
		}
		if (StatemachineViewsRepository.Region.Properties.keywords == event
				.getAffectedEditor()) {
		...
		if (StatemachineViewsRepository.Region.Properties.name == event
				.getAffectedEditor()) {
			region.setName((java.lang.String) EEFConverterUtil
					.createFromString(EcorePackage.Literals.ESTRING,
							(String) event.getNewValue()));
		}
	}
	...
	public void updatePart(final Notification msg) {
		...
		if (this.editingPart.isVisible()) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description()
					...
					&& (regionPart != null)
					&& this.isAccessible(StatemachineViewsRepository.Region.Properties.description)) {
				if (msg.getNewValue() != null) {
					regionPart.setDescription(EcoreUtil.convertToString(
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Keywords()
					...
					&& msg.getNotifier().equals(this.semanticObject)
					&& (regionPart != null)
					&& this.isAccessible(StatemachineViewsRepository.Region.Properties.keywords)) {
				if (msg.getNewValue() instanceof EList<?>) {
					regionPart.setKeywords((EList<?>) msg.getNewValue());
				} else if (msg.getNewValue() == null) {
					regionPart.setKeywords(new BasicEList<Object>());
				} else {
					final BasicEList<Object> newValueAsList = new BasicEList<Object>();
					newValueAsList.add(msg.getNewValue());
					regionPart.setKeywords(newValueAsList);
				}
			}
			...
			if (StateMachinePackage.eINSTANCE.getNamedElement_Name().equals(
					...
					&& msg.getNotifier().equals(this.semanticObject)
					&& (regionPart != null)
					&& this.isAccessible(StatemachineViewsRepository.Region.Properties.name)) {
				if (msg.getNewValue() != null) {
					regionPart.setName(EcoreUtil.convertToString(
							EcorePackage.Literals.ESTRING, msg.getNewValue()));
				} else {
					regionPart.setName("");
				}
