 ...
 * 
 */
public class InitialNodeInitialNodePropertiesEditionComponent extends
	 ...
	 * 
	 */
	public InitialNodeInitialNodePropertiesEditionComponent(
			final PropertiesEditingContext editingContext,
			final EObject initialNode, final String editing_mode) {
		super(editingContext, initialNode, editing_mode);
		this.parts = new String[] { INITIALNODE_PART };
		this.repositoryKey = GraalViewsRepository.class;
		this.partKey = GraalViewsRepository.InitialNode.class;
	}
	...
	public void initPart(final Object key, final int kind, final EObject elt,
		...
		if ((this.editingPart != null) && (key == this.partKey)) {
			...
			final InitialNodePropertiesEditionPart initialNodePart = (InitialNodePropertiesEditionPart) this.editingPart;
			// init values
			if (this.isAccessible(GraalViewsRepository.InitialNode.Properties.description)) {
				initialNodePart.setDescription(EcoreUtil.convertToString(
						EcorePackage.Literals.ESTRING,
						initialNode.getDescription()));
			}
			if (this.isAccessible(GraalViewsRepository.InitialNode.Properties.outgoingTransitions)) {
			...
			if (this.isAccessible(GraalViewsRepository.InitialNode.Properties.incomingTransitions)) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		final InitialNode initialNode = (InitialNode) this.semanticObject;
		if (GraalViewsRepository.InitialNode.Properties.description == event
				.getAffectedEditor()) {
			initialNode.setDescription((java.lang.String) EEFConverterUtil
					.createFromString(EcorePackage.Literals.ESTRING,
							(String) event.getNewValue()));
		}
		if (GraalViewsRepository.InitialNode.Properties.outgoingTransitions == event
				.getAffectedEditor()) {
		...
		if (GraalViewsRepository.InitialNode.Properties.incomingTransitions == event
				.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof Transition) {
	...
	public void updatePart(final Notification msg) {
		...
		if (this.editingPart.isVisible()) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description()
					...
					&& (initialNodePart != null)
					&& this.isAccessible(GraalViewsRepository.InitialNode.Properties.description)) {
			...
			if (GraalPackage.eINSTANCE.getNode_OutgoingTransitions().equals(
					msg.getFeature())
					&& this.isAccessible(GraalViewsRepository.InitialNode.Properties.outgoingTransitions)) {
				initialNodePart.updateOutgoingTransitions();
			}
			if (GraalPackage.eINSTANCE.getNode_IncomingTransitions().equals(
					msg.getFeature())
					&& this.isAccessible(GraalViewsRepository.InitialNode.Properties.incomingTransitions)) {
