...
/**
 * Custom class used to provide the right label and image when plugging EEF
 * properties views on a Viewpoint table
 * 
 * @author sthibaudeau
 * 
 */
public class CustomEEFLabelProvider extends EEFLabelProvider {
	...
	public Image getImage(final Object element) {
		final Object unwrappedElement = this.unwrap(element);
	...
	private Object unwrap(final Object element) {
		if (element instanceof IStructuredSelection) {
			return this.unwrap(((IStructuredSelection) element)
					.getFirstElement());
		}
		if (element instanceof IAdaptable) {
			final EObject eObject = (EObject) ((IAdaptable) element)
